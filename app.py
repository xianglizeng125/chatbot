...
MAX_LEN = 100
GOOGLE_DRIVE_ZIP_ID = "13kZQFNNE8BI9Ix519l6fZ9lxIpKr4m9J"  # ID ZIP terbaru
ZIP_PATH = "nlp_assets.zip"
EXTRACT_PATH = "nlp_assets"

# ====== DEFINE CUSTOM FUNCTION FOR LAMBDA LAYER ======
    ...
    if not os.path.exists(EXTRACT_PATH):
        with st.spinner("📥 Downloading model & tokenizer..."):
            ...
            try:
                ...
                st.error(f"❌ Error downloading/extracting: {e}")
...
if submitted and user_input:
    ...
    elif category and is_category_input and not explicit_negative:
        is_negative = False
    ...
    if matched_menu:
        ...
        if is_negative:
            ...
            response = f"🙅‍♂️ Avoiding <strong>{category.replace('_', ' ').title()}</strong>? Try these instead:"
        else:
            recommended = menu_stats[menu_stats["menu"].isin(matched)].sort_values("score", ascending=False).head(3)
...
for sender, message in st.session_state.chat_history:
    st.markdown(f"**{sender}:** {message}", unsafe_allow_html=(sender == "Bot"))
