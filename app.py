...
def download_and_load_assets():
    ...
    bert_model = TFDistilBertModel.from_pretrained(os.path.join(EXTRACT_PATH, "bert_model"), local_files_only=True)
...
if submitted and user_input:
    ...
    if matched_menu:
        if is_negative:
            ...
            response = sentiment_note + "🤔 Hmm, not sure about that menu. Here are some favorites:"
    elif category:
        ...
        if is_negative:
            ...
            response = sentiment_note + f"🙅‍♂️ Skipping <strong>{category.replace('_',' ')}</strong>? Try these:"
        else:
            recommended = menu_stats[menu_stats["menu"].isin(matched)].sort_values("score", ascending=False).head(3)
            response = sentiment_note + f"🔥 Great taste! Here are top <strong>{category.replace('_',' ')}</strong> picks:"
    else:
        recommended = menu_stats.sort_values("score", ascending=False).head(3)
        response = sentiment_note + "🤔 Couldn't find anything specific, but here's our top picks:"
